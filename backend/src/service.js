...
export const addChannel = (authUserId, name, priv, description) =>
  channelLock((resolve, reject) => {
    ...
    channels[channelId] = newChannelPayload(
      name,
      authUserId,
      priv,
      description
...
export const joinChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
    ...
    } else if (channels[channelId].private === true || channels[channelId].private === "true") {
...
export const leaveChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    if (channels[channelId].creator === parseInt(authUserId, 10)) {
      return reject(
        new InputError(
          "Channel creators must delete the channel instead of leaving it"
        )
      );
    }
...
export const deleteChannel = (authUserId, channelId) =>
  channelLock((resolve, reject) => {
    assertChannelMember(authUserId, channelId);
    if (channels[channelId].creator !== parseInt(authUserId, 10)) {
      return reject(
        new AccessError("Only the channel creator can delete this channel")
      );
    }
...
export const updateProfile = (authUserId, email, password, name, bio, image) =>
  userLock((resolve, reject) => {
    ...
    const existingUserId = email ? getUserIdFromEmail(email) : undefined;
    if (
      email &&
      existingUserId !== undefined &&
      existingUserId !== authUserId.toString()
    ) {
...
export const sendMessage = (authUserId, channelId, message, image) =>
  channelLock((resolve, reject) => {
    ...
    const nextMessage = message === undefined ? "" : message;
    const nextImage = image === undefined ? undefined : image;
    if (isBlankMessage(nextMessage) && (nextImage === undefined || nextImage === "")) {
      return reject(new InputError("Message cannot be empty"));
    }
...
export const editMessage = (
  ...
  channelLock((resolve, reject) => {
    ...
    assertChannelMember(authUserId, channelId);
    const messageIndex = findMsgIdxInChannel(channelId, messageId);
    const existingMessage = channels[channelId].messages[messageIndex];
    if (existingMessage.sender !== parseInt(authUserId, 10)) {
      return reject(
        new AccessError("You can't edit a message you didn't send")
      );
    }

    const nextMessage = newMessage === undefined ? existingMessage.message : newMessage;
    const nextImage = newImage === undefined ? existingMessage.image : newImage;

    if (isBlankMessage(nextMessage) && (nextImage === undefined || nextImage === "")) {
      return reject(new InputError("Message cannot be empty"));
    }

    if (nextMessage === existingMessage.message && nextImage === existingMessage.image) {
      return reject(
        new InputError("Edited message must be different from the current message")
      );
    }

    existingMessage.message = nextMessage;
    existingMessage.image = nextImage;
